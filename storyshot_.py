"""
StoryShot Orchestrator (storyshot_.py) - MVP Version 5 (Includes Assembly Stage)
...
8. NEW: Orchestrating Final Video/Audio Assembly using ffmpeg_assembler_.py.
...
Usage:
   python storyshot_.py <path_to_transcript.txt> [--force-regenerate] [--skip-images] [--skip-videos] [--skip-assembly]
...
Cache Structure (`cache_data/cache.json`): (Remains the same structure)
{
  "normalized_sentence_key_1": {
    ...
    // Added by assembler (optional, not currently implemented):
    // "assembly_output": { "final_clip_path": "..." }
  }, ...
}
"""
...
try:
    ...
    from ffmpeg_assembler_ import FFmpegAssembler, FFmpegAssemblyError # Added
...
FFMPEG_LIST_SUBDIR = TEMP_DIR / "ffmpeg_concat_lists" # Added for assembler
FINAL_CLIPS_SUBDIR = TEMP_DIR / "final_clips"       # Added for assembler
...
class StoryShotOrchestrator:
    ...
    def __init__(self, engine: OpenRouterEngine):
        ...
        self.assembler = FFmpegAssembler(final_movie_path=self.final_movie_path) # Added Assembler
    ...
    def _reset_run_metrics(self):
        ...
        # TTS, SB, IMG, VID metrics remain the same...
        # TTS Metrics
        ...
        self.current_run_cost_vid = Decimal("0.0")
        ...
        # Assembly Metrics (ASM) - Added
        self.assembly_start_time = None
        self.assembly_end_time = None
        self.assembly_success = None
        self.final_movie_output_path = None

        # Combined Metrics
        self.current_run_cost_total = Decimal("0.0")

    # _load_cache, _save_cache, _normalize_sentence, _get_sentence_hash,
    # _get_audio_output_path, _get_image_output_path, _get_video_output_path,
    # load_transcript, tokenize_sentences
    ...
    async def process_transcript(self, transcript_text: str, force_regenerate: bool = False, skip_images: bool = False, skip_videos: bool = False, skip_assembly: bool = False): # Added skip_assembly
        """
        ...
        logger.info(f"--- Starting Transcript Processing Run (ForceR: {force_regenerate}, SkipI: {skip_images}, SkipV: {skip_videos}, SkipA: {skip_assembly}) ---") # Updated log
        ...
        # Get ordered keys *once* for consistent processing order
        ...
        for i, normalized_key in enumerate(ordered_sentence_keys):
            sentence = original_sentences[i] # Get original sentence text
            ...
            logger.debug(f"TTS Prep: Sentence {i+1}/{len(original_sentences)}.")
        ...
        for i, normalized_key in enumerate(ordered_sentence_keys):
            ...
            cache_entry = self.cache[normalized_key] # Assume it exists from TTS stage
            ...
            logger.debug(f"SB Prep: Sentence {i+1}/{len(original_sentences)}.")
        ...
        if skip_images: logger.info("--- Stage: Prompt-to-Image (Skipped via flag) ---")
            ...
            for i, normalized_key in enumerate(ordered_sentence_keys):
                ...
                for p_idx, prompt_text in enumerate(prompts):
                    ...
                    logger.debug(f"IMG Prep: Sentence {i+1}/{len(original_sentences)}, Prompt {p_idx+1}/{len(prompts)}.")
            ...
            if img_tasks:
                ...
                for i, result in enumerate(results):
                    ...
                    if existing_video_output is not None: base_img_output["video_output"] = existing_video_output
        ...
        if skip_videos: logger.info("--- Stage: Image-to-Video (Skipped via flag) ---")
            ...
            for i, normalized_key in enumerate(ordered_sentence_keys):
                ...
                if not cache_entry or 'storyboard_prompts' not in cache_entry or not cache_entry['storyboard_prompts']: continue
                sb_data = cache_entry['storyboard_prompts']
                if sb_data.get('status') != 'success' or not sb_data.get('image_outputs'): continue
                image_outputs = sb_data['image_outputs']
                prompts = sb_data.get('prompts', [])
                if len(prompts) != len(image_outputs):
                    logger.warning(f"Prompt/image mismatch for sentence {i+1}. Skipping video gen."); continue
                ...
                for p_idx, img_output in enumerate(image_outputs):
                    ...
                    if not image_path or not Path(image_path).exists(): logger.warning(f"Image path missing/not found ({image_path}) for s{i+1}, p{p_idx+1}. Skip video."); continue
                    if not prompt_text: logger.warning(f"Original prompt missing for s{i+1}, p{p_idx+1}. Skip video."); continue
                    ...
                    task = asyncio.create_task( self.video_gen_handler.generate_video(image_path, prompt_text, output_video_path), name=f"VID_s{i+1}_p{p_idx+1}_{normalized_key[:6]}" )
            ...
            if vid_tasks:
                ...
                for i, result in enumerate(results):
                    ...
                    try: img_output_slot = cache_entry['storyboard_prompts']['image_outputs'][p_idx]; assert isinstance(img_output_slot, dict)
                    except (IndexError, KeyError, TypeError, AssertionError): logger.error(f"Cache error for VID task {task_name}. Cannot store result."); continue
                    ...
                    if img_output_slot:
                         if isinstance(result, Exception):
                            ...
                            if result.get("status") == "success":
                                ...
                                img_output_slot['video_output'] = {"status": "success", "video_path": result.get('video_path'), "cost": float(vid_cost), "inference_time": vid_inf_time, "error_message": None}
                                ...
                                img_output_slot['video_output'] = {"status": "failed", "cost": float(vid_cost), "inference_time": vid_inf_time, "error_message": error_msg}
                                self.current_run_cost_vid += vid_cost ; self.current_run_cost_total += vid_cost
                         else:
        ...
        # ==================================
        # --- Step 5: Video/Audio Assembly --- # NEW STAGE
        # ==================================
        if skip_assembly: logger.info("--- Stage: Video/Audio Assembly (Skipped via flag) ---")
        elif skip_videos or skip_images: logger.info("--- Stage: Video/Audio Assembly (Skipped because image/video gen was skipped) ---")
        else:
            logger.info("--- Stage: Video/Audio Assembly ---")
            ...
            try:
                ...
                if final_movie_file and final_movie_file.exists():
                    ...
                    logger.error("Assembly process finished but final movie file was not found.")
    ...
    def log_run_summary(self):
        ...
        cost_vid_str = f"{self.current_run_cost_vid:.4f}" # Note: Still placeholder for accurate video cost
        ...
        print(f"  Estimated Cost (This Run): ${cost_vid_str}") # Still placeholder value
        print("-" * summary_width)
        # Assembly Summary --- Added ---
        print("[Video/Audio Assembly Stage]")
        if self.assembly_start_time is None:
            print("  Status: Skipped")
        else:
            assembly_duration = (self.assembly_end_time - self.assembly_start_time) if self.assembly_end_time else 0.0
            status_str = "Success" if self.assembly_success else "Failed" if self.assembly_success is False else "Unknown"
            ...
            if self.assembly_success and self.final_movie_output_path:
                print(f"  Final Output: {self.final_movie_output_path}")
                 ...
                 print("  No final movie file generated due to errors.")
        print("=" * summary_width + "\n")

        # Log lifetime costs... (same as before)
        try:
            ...
            handler_vid_stats = self.video_gen_handler.get_stats(); handler_vid_total_cost = handler_vid_stats.get('total_estimated_cost', Decimal('0.0'))
...
async def run_orchestrator(args):
    ...
    try:
        ...
        await orchestrator.process_transcript(
            ...
            args.skip_images,
            args.skip_videos,
            args.skip_assembly # Pass skip_assembly flag
        )

    except FileNotFoundError: sys.exit(1)
    except (TextToSpeechError, SentenceToStoryboardError, OpenRouterError, PromptToImageError, ImageToVideoError, FFmpegAssemblyError) as e: # Added FFmpegAssemblyError
...
if __name__ == "__main__":
    ...
    parser.add_argument("--skip-videos", action="store_true", help="Skip the image-to-video generation stage.")
    parser.add_argument("--skip-assembly", action="store_true", help="Skip the final video/audio assembly stage.") # Added
    args = parser.parse_args()

    # --- Pre-computation Checks ---
    print("--- Running Pre-computation Checks ---")
    checks_passed = True
    # Check .env and Keys (same as before)
    ...
    # ================================================
    # --- Check FFmpeg presence (SIMPLIFIED) ---
    # ================================================
    print("Info: Checking for FFmpeg...")
    ffmpeg_path = shutil.which("ffmpeg") # Use shutil.which to find in PATH
    if ffmpeg_path:
        print(f"Info: Found FFmpeg executable at: {ffmpeg_path}")
        # Optional: Could add a synchronous subprocess.run check here if needed
        # try:
        #     result = subprocess.run(["ffmpeg", "-version"], capture_output=True, text=True, check=True, timeout=5)
        # except Exception as ffmpeg_err:
        #      print(f"Warning: Found ffmpeg but '-version' check failed: {ffmpeg_err})")
        #      # checks_passed = False # Decide if this is critical
    else:
        print("Error: FFmpeg executable not found in system PATH. Assembly will fail.")
        print("       Please install FFmpeg (e.g., 'sudo apt install ffmpeg' or download from ffmpeg.org)")
        print("       and ensure its directory is included in your system's PATH environment variable.")
        checks_passed = False
    # ================================================

    # Check Transcript file (same as before)
    ...
    if not transcript_path.is_file():
         ...
         if transcript_path.name.lower() in ["sample_transcript.txt", "transcript.txt"]:
             ...
             try:
                 ...
                 # Allow continuing, user needs to edit the dummy file.
             except Exception as e: print(f"Failed to create dummy file: {e}"); checks_passed = False
         else: checks_passed = False

    # Check Directories (ensure assembler dirs are checked too)
    try:
        CACHE_DIR.mkdir(exist_ok=True); TEMP_DIR.mkdir(exist_ok=True)
        AUDIO_SUBDIR.mkdir(exist_ok=True); IMAGES_SUBDIR.mkdir(exist_ok=True); VIDEOS_SUBDIR.mkdir(exist_ok=True)
        FFMPEG_LIST_SUBDIR.mkdir(exist_ok=True); FINAL_CLIPS_SUBDIR.mkdir(exist_ok=True)
